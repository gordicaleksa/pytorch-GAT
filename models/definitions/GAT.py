...
class GATLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, layer_type, concat=True, activation=nn.ELU(),
        ...
        if layer_type == LayerType.IMP4:
            self.linear_param = nn.Parameter(torch.Tensor(num_of_heads * num_out_features, num_in_features))
        ...
        if bias and concat:
            if layer_type == LayerType.IMP4:
                self.bias = nn.Parameter(torch.Tensor(num_of_heads * num_out_features, 1))
            else:
                self.bias = nn.Parameter(torch.Tensor(num_of_heads * num_out_features))
        elif bias and not concat:
            if layer_type == LayerType.IMP4:
                self.bias = nn.Parameter(torch.Tensor(num_out_features, 1))
            else:
    ...
    def init_params(self, layer_type):
        ...
        """
        if layer_type == LayerType.IMP4:
            nn.init.xavier_uniform_(self.linear_param)
...
class GATLayerImp4(GATLayer):
    ...
    def forward(self, data):
        ...
        # shape = (FIN, N) where N - number of nodes in the graph, FIN number of input features per node
        # We apply the dropout to all of the input node features (as mentioned in the paper)
        in_nodes_features = self.dropout(in_nodes_features)

        # shape = (NH, FOUT, N) where NH - number of heads, FOUT number of output features per head
        # We project the input node features into NH independent output features (one for each attention head)
        nodes_features_proj = torch.mm(self.linear_param, in_nodes_features).view(self.num_of_heads, self.num_out_features, -1)
        ...
        edge_features = torch.cat((nodes_features_proj_lifted, nodes_features_proj.index_select(2, edge_index[1])), dim=1)
        ...
        size[-1] = num_of_nodes  # shape = (NH, FOUT, N)  # todo: replace -1 with var
        ...
        if self.concat:
            out_nodes_features = out_nodes_features.view(self.num_of_heads * self.num_out_features, -1)
        else:
            out_nodes_features = out_nodes_features.mean(dim=0)
    ...
    def neighborhood_aware_softmax(self, scores_per_edge, trg_index, num_of_nodes):
        ...
        return attentions_per_edge.unsqueeze(-2)  # so that we can do element-wise multiplication with proj features

    def sum_edge_scores_neighborhood_aware(self, exp_scores_per_edge, trg_index, num_of_nodes):
        # The shape must be the same as in exp_scores_per_edge (required by scatter_add_) i.e. from N -> (NH, N)
        trg_index_broadcasted = self.broadcast(trg_index, exp_scores_per_edge)

        # shape = (NH, N), where N is the number of nodes and NH the number of attention heads
